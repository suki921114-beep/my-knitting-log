  ...
  return (
    <div>
      <PageHeader
        ...
        right={
          <Link to="/library/yarns/new" className="inline-flex items-center gap-1 rounded-full bg-primary px-3 py-2 text-xs font-medium text-primary-foreground shadow-soft">
      ...
      {filtered.length === 0 ? (
        ...
        <ul className="space-y-2">
          {filtered.map(s => {
            ...
            return (
              <li key={s.yarn.id}>
                <Link to={`/library/yarns/${s.yarn.id}`} className="card-soft block p-3.5">
