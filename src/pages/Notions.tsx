  ...
  return (
    <div>
      <PageHeader title="부자재" back right={
        <Link to="/library/notions/new" className="inline-flex items-center gap-1 rounded-full bg-primary px-3 py-2 text-xs font-medium text-primary-foreground shadow-soft">
      ...
      {filtered.length === 0 ? (
        ...
        <ul className="space-y-2">
          {filtered.map(n => (
            <li key={n.id}>
              <Link to={`/library/notions/${n.id}/edit`} className="card-soft flex items-center justify-between p-3.5">
                <div>
                  <div className="font-medium text-ink">{n.name}</div>
                  <div className="text-xs text-muted-foreground">{n.kind} {n.shop && `· ${n.shop}`}</div>
                </div>
